...
- Moves account-protection capacity decisions into a Provider-scoped SQLite `BEGIN IMMEDIATE` critical section with fail-closed busy handling, a 200 ms reservation-handle SQLite busy timeout, and a Linux release gate requiring sustained writer-lock failure to return below 650 ms so at least 100 ms remains before CPA's 750 ms scheduler deadline. It also adds transactional terminal release, TTL/crash recovery, and 2/4/8-process stress gates. Reservation and usage aliases are evaluated separately so duplicate credentials keep strict file-scoped hard limits while ambiguous legacy usage is conservatively attributed instead of bypassing soft Token demotion; the scheduler uses the store's known database path so a concurrent usage refresh cannot force a new primary WAL-negotiating connection under writer contention, and the reservation handle itself omits WAL negotiation.
...
- Narrows official binary support and Release workflow builds to glibc Linux on amd64 and arm64. The closed release bundle now contains two Linux archives, two matching SPDX SBOMs, and `checksums.txt`; non-Linux binaries are not built, tested, or published by the release workflow.
