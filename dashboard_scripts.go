...
function cancelInvalidAuthOAuth(){
  invalidAuthOAuthGeneration++;
  if(invalidAuthOAuthTimer)clearTimeout(invalidAuthOAuthTimer);
...
function pollInvalidAuthOAuth(management,state,row,before,startedAt,oldFile,oldEmail,generation){
...
async function handleInvalidAuthOAuthSuccess(management,row,before,startedAt,oldFile,oldEmail,generation){
...
function updateAuthImportControls(){
...
async function previewAuthImport(){
  if(authImportReadPending){setAuthImportStatus('文件仍在读取，请稍候。','warn');return}
...
async function writeBatchProxy(proxy,donePrefix,failPrefix,progressPrefix){
  ...
  try{
    setBatchProxyStatus('正在读取 Codex 认证文件...','');
    const files=await fetchAuthFilesForBatch(key);
    if(!files.length){batchProxyUrlEl.value='';setBatchProxyStatus('没有找到可写入的 Codex 认证文件。','warn');return}
    let ok=0,failed=0;
    for(const file of files){
...
function reliableThroughputSample(r){if(r&&r.generate===false)return false; const latency=Number(r.latency_ms||0),ttft=Number(r.ttft_ms||0),ms=Math.max(latency,ttft),out=Number(r.output_tokens||0); return out>0&&ms>=1000&&!(latency===ttft&&out>=1000&&ms<5000)}
